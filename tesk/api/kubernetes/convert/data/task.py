...
class Task:
    """Task is a composite that represents Kubernetes object's graph of a single TES task."""
    ...
    def get_executor_name(self, executor_index: int) -> str:
        taskmaster_name = self.taskmaster.get_job_name()
        return f"{taskmaster_name}{self.constants.job_name_exec_prefix}{str(executor_index).zfill(self.constants.job_name_exec_no_length)}"

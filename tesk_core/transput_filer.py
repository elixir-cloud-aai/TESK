...
class FTPTransput(Transput):
  ...
  def upload_file(self):
    try:
      ...
      logging.error('Unable to store file '+self.path+' at FTP location /'+self.ftp_path)
      raise
      return 1
  ...
  def download_file(self):
    logging.debug('downloading ftp file: '+self.url+' target: '+self.path)
...
def process_file(ttype, filedata):
  ...
  try:
    ...
    logging.error('Could not determine protocol for url: '+filedata['url'])
    return 1
